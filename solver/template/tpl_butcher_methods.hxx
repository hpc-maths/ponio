...
namespace ode::butcher
{
    ...
    {
        {
            "adaptive_" if 'b2' in rk else ""
        }
    }
    butcher_tableau<{ { rk.A | length } }, value_t>
    {
        ...
        static constexpr char const* id       = "{{ rk.id }}";
    ...
    _t = runge_kutta::explicit_rk_butcher<butcher_{ { rk.id } }<value_t>>;
    ...
    _t = []( Exp_t exp_, double tol = ponio::default_config::tol )
    {
        return lawson::make_lawson<butcher_{ { rk.id } }<value_t>, Exp_t>( exp_, tol );
    };

    template <typename Exp_t>
    auto l{ { rk.id } }( Exp_t exp_, double tol = ponio::default_config::tol )
    {
     ...
     */
    template <typename value_t>
    using explicit_rk_tuple = std::tuple<{ { list_erk | sformat( "{}_t<value_t>", attribute = "id" ) | join( ", " ) } }>;

    /**
     * @brief Type of tuple that contains all Lawson methods of ponio
     */
    template <typename value_t, typename Exp_t>
    using lawson_rk_tuple = std::tuple<{ { list_erk | sformat( "decltype(l{}_t<value_t, Exp_t>)", attribute = "id" ) | join( ", " ) } }>;
...

    {
        ...
        { { t } } { { " , " if not loop.last else "" } } { % -endfor % } > a;
        std::tuple <
        {% for t in rk.b.type -%
        }
        { { t } } { { " , " if not loop.last else "" } } { % -endfor % } > b;
    ...
    _t = exp_runge_kutta::explicit_exp_rk_butcher<butcher_{ { rk.id } }<value_t, linear_t>>;
