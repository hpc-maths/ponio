...
main()
{
    ...
    double const tend = 0.01; // tini + 5 * dt; // 0.5;
    ...
    // for ( std::size_t N = 5; N < 100; ++N )
    {
        // double dt = ( tend - tini ) / static_cast<double>( N );
        double dt = 1e-4;

        yend = ponio::solve( pb_heat, ponio::runge_kutta::rock::rock2(), yini, tspan, dt, observer::null_observer() );
