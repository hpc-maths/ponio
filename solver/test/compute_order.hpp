...
order_lv ()
{
  ...
  auto pb = [=]( T t , state_t const& u ) -> state_t {
    return {
  ...
  std::vector<T> dts = {0.25,0.125,0.1,0.075,0.05};
  std::vector<T> relative_error, ldts;

  for ( auto dt : dts ) {
    state_t u_end = ode::solve(pb, Algorithm_t(), u_ini, t_span, dt,  [](T,state_t const&,T){});
    relative_error.push_back( std::log10(std::abs(V(u_end)-V_ini)/V_ini) );
    ldts.push_back(std::log10(dt));
  }

  auto [a,b] = mayor_method(ldts,relative_error);
