...
class rk_butcher:
    ...
    def stage_order(self):
        ...
        if self._stage_order is None:
            self._stage_order = sum([
                sum([
                    sum([self.A[i, j]*self.c[j]**(s-1)
                        for j in range(self.nstages)]) == sp.Rational(1, s)*self.c[i]**s
                    for i in range(self.nstages)
    ...
    def is_approched_method(self):
        if self._is_approched_method is None:
            self._is_approched_method = any(
                [type(x) is sp.Float for x in itertools.chain(self.A, self.b, self.c)])
    ...
    def stability_function(self, embedded=False):
        ...
        if self._R is None:
            self._R = stab_func(
                self.A, self.b, approched_method=self.is_approched_method)
        return self._R
    ...
    def scheme(self, latex=True, lawson=False):
        if latex:
            ...
            if lawson:
                ...
                un, unp1, ubnp1 = (sp.MatrixSymbol(sp.latex(ui), 3, 1)
                                   for ui in (un, unp1, ubnp1))
        else:
            ...
            if lawson:
                ...
                un, unp1, ubnp1 = (sp.MatrixSymbol(str(ui), 3, 1)
                                   for ui in (un, unp1, ubnp1))
        ...
        if lawson:
            def mexp(expr):
                ...
                if latex:
                    ...
                    expr_str = r"exp({})".format(
                        str(expr.simplify().evalf()).replace("1.0*", ""))
            ...
            def N(t, u):
                if latex:
                    expr_str = r"N\left({}, {}\right)".format(
                        sp.latex(t), sp.latex(u.simplify()))
                else:
                    expr_str = r"N({}, {})".format(str(t), str(u.simplify()))
                return sp.MatrixSymbol(expr_str, *u.shape)

            def f(t, u): return mexp((tn-t)*L)*N(t, mexp((t-tn)*L)*u)
    ...
    def code(self, lawson=False):
        ...
        if lawson:
            args = {"L": "linar part, a scalar or a matrix",
                    "N": "non-linear part, a function N(t,u)"}
        args.update(
            [
                ("tn", "current time"),
        ...
        r = ["def {funcname}( {args} ):".format(funcname=func_name(
            self.id, lawson), args=", ".join(args.keys()))]
    ...
    {warning}
    source: https://hpc-maths.github.io/ponio/#{meth.id}
\"\"\"""".format(
            meth=self,
            embedded="" if not self.is_embedded else "Embedded ",
            lawson="" if not lawson else "Lawson ",
            arguments="\n    ".join(
                [f"{{k:{args_len}}}    {{v}}".format(k=k, v=v) for k, v in args.items()]),
            ...
            output="{} solution at time tn".format(str(scheme[-1].lhs)) if not self.is_embedded else "(t,u,dt,error) tuple where t is the time of the {unp1}, {ubnp1}) tuple of solutions at time tn of order {order} and {orderbis}".format(
                unp1=str(scheme[-2].lhs), ubnp1=str(scheme[-1].lhs), order=self.order, orderbis=self.order-1),
...
if error > tol:
    ...
    return (tn+dt,{unp1},ndt,error)""".format(unp1=str(scheme[-2].lhs), ubnp1=str(scheme[-1].lhs), power=1./self.order)
            ...
            r[-1] = r[-1].replace("{unp1} =".format(
                unp1=str(scheme[-1].lhs)), "return")
...
def analysis_butcher(rk):
    ...
    pattern = re.compile(
        "\\\\left\(\\\\frac\{(?P<numerator>[^{}]+)\}\{(?P<denominator>[^{}]+)\}\\\\right\)")
    ...
    analysis['lawson_scheme'] = [
        "{} = {}".format(sp.latex(eq.lhs), pattern.sub(
            lambda m: m.group(0)[6:-7], sp.latex(eq.rhs)))
...
parser = argparse.ArgumentParser(
    description="analysis of Runge-Kutta method script")
parser.add_argument('FILE', nargs='*',
                    help="File which contain a Butcher tableau")
parser.add_argument('-o', '--output', type=str,
                    help="Output directory where store Runge-Kutta scheme analysis and static API on stability domain and order stars")
parser.add_argument('-v', '--verbose', action='store_true',
                    help="Verbose mode")
parser.add_argument('-s', '--standalone', action='store_true',
                    help="All analysis in one file (by default each analysis of each method is in its own file)")
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(args.FILE):
        ...
        zmax = 2+3.5j
        N = (512, 512)
        stability_domain = {'xmin': np.real(zmin), 'xmax': np.real(
            zmax), 'ymin': np.imag(zmin), 'ymax': np.imag(zmax)}
        stability_domain['data'] = evalf_Cdomain(sp.symbols(
            "z"), rk.stability_function(), zmin, zmax, N).tolist()

        if rk.is_embedded:
            stability_domain['data_embedded'] = evalf_Cdomain(sp.symbols(
                "z"), rk.stability_function(embedded=True), zmin, zmax, N).tolist()

        # relative error (or precision) thanks to Laurent François
        relative_error = {'xmin': np.real(zmin), 'xmax': np.real(
            zmax), 'ymin': np.imag(zmin), 'ymax': np.imag(zmax)}
        relative_error['data'] = evalf_Cdomain(sp.symbols(
            "z"), rk.relative_error_function(), zmin, zmax, N).tolist()

        # order star
        zmin = -3.5-3.5j
        zmax = 3.5+3.5j
        N = (512, 512)
        order_star = {'xmin': np.real(zmin), 'xmax': np.real(
            zmax), 'ymin': np.imag(zmin), 'ymax': np.imag(zmax)}
        order_star['data'] = evalf_Cdomain(sp.symbols(
            "z"), rk.order_star_function(), zmin, zmax, N).tolist()
